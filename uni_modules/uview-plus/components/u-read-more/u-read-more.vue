<template>
	<view class="u-read-more">
		<view class="u-read-more__content" :style="{
				height: isLongContent && status === 'close' ? $u.addUnit(showHeight) : $u.addUnit(contentHeight,'px'),
		...
		<view class="u-read-more__toggle" :style="[innerShadowStyle]" v-if="isLongContent">
			<slot name="toggle">
				<view class="u-read-more__toggle__text" @tap="toggleReadMore">
					<u--text :text="status === 'close' ? closeText : openText" :color="color" :size="fontSize"
						:lineHeight="fontSize" margin="0 5px 0 0"></u--text>
		...
		methods: {
			async init() {
				this.getContentHeight().then(height => {
					//2024-4-9 
					//wp
					//这里获取的是px单位的高度，如果配置了uni.$u.config.unit为rpx,单位对不上，所以要给addUnit添加单位参数px
